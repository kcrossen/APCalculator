...
/* @krc@ S_FUNC int filescan(FILEID id, int count, VALUE **vals); @krc@ */
S_FUNC VALUE f_eval(VALUE *vp);
/* @krc@ S_FUNC VALUE f_fsize(VALUE *vp); @krc@ */
/* @krc@ S_FUNC int malloced_putenv(char *str); @krc@ */
...
/* @krc@ E_FUNC int idungetc(FILEID id, int ch); @krc@ */
...
f_display(int count, VALUE **vals)
{
	...
	if (count > 0) {
		if (vals[0]->v_type != V_NUM || qisfrac(vals[0]->v_num) ||
			...
			err_str("Out-of-range arg for display ignored\n");
		else
...
f_errmax(int count, VALUE **vals)
{
	...
	if (count > 0) {
		...
		if (vp->v_type != V_NUM || qisfrac(vp->v_num) ||
			...
			err_str("Out-of-range arg for errmax ignored\n");
		} else {
...
f_stoponerror(int count, VALUE **vals)
{
	...
	if (count > 0) {
		...
		if (vp->v_type != V_NUM || qisfrac(vp->v_num) ||
		    zge31b(vp->v_num->num) || zltnegone(vp->v_num->num)) {
			err_str("Out-of-range arg for stoponerror ignored\n");
		} else {
...
f_printf(int count, VALUE **vals)
{
	...
	cp = user_printf(vals[0]->v_str->s_str, count - 1, vals + 1);
	if (strlen(cp) == 0)
...
f_strprintf(int count, VALUE **vals)
{
	...
	cp = user_printf(vals[0]->v_str->s_str, count - 1, vals + 1);
	if (strlen(cp) == 0) {
		/* @krc@ free(math_getdivertedio()); */
		return error_value(E_STRPRINTF2);
	}
	/* @krc@ cp = math_getdivertedio(); */
	result.v_type = V_STR;
	result.v_str = makenewstring(cp);
	free(cp);
	return result;
...
f_join(int count, VALUE **vals)
{
	...
	while (count-- > 0) {
		if (vals[0]->v_type != V_LIST) {
			listfree(lp);
			out_str("Non-list argument for join\n");
			res.v_type = V_NULL;
			return res;
		}
...
f_inputlevel (void)
{
	...
	result.v_num = itoq((long) 0 /* @krc@ inputlevel() */);
	return result;
}
...
main(int argc, char *argv[])
{
	...
	for (bp = builtins; bp->b_name; bp++) {
		out_fmt("%-9s ", bp->b_name);
		if (bp->b_maxargs == IN)
			out_fmt("%d+    ", bp->b_minargs);
		else if (bp->b_minargs == bp->b_maxargs)
			out_fmt("%-6d", bp->b_minargs);
		else
			out_fmt("%d-%-4d", bp->b_minargs, bp->b_maxargs);
		out_fmt("%s\n", bp->b_desc);
	}
	out_str("\n");
	return 0;	/* exit(0); */
...
showerrors(void)
{
	...
	if (nexterrnum == E_USERDEF)
		out_str("No new error-values created\n");
