...
/* @krc@ S_FUNC void do_showcommand(void); */
...
int
execute_commands(BOOL toplevel) {
...
getcommands(BOOL toplevel)
{
	...
	for (;;) {
		...
		default:
			...
			freefunc(curfunc);
			if (abort_now) {
                /* @krc@ Input is effectively equivalent to a terminal
				if (!stdin_tty)
					run_state = RUN_EXIT;
				...
				if (calc_use_scanerr_jmpbuf != 0) {
					...
					err_str("calc_scanerr_jmpbuf not setup, exiting code 30\n");
...
getshowstatement(void)
{
	...
	case T_STATIC:
		arg = 14; break;
	default:
		out_str("SHOW command to be followed by at least ");
		out_str("four letters of one of:\n");
		out_str("\tblocks, builtin, config, constants, ");
		out_str("custom, errors, files, functions,\n");
		out_str("\tglobaltypes, objfunctions, objtypes, "
		       "opcodes, sizes, ");
		out_str("realglobals,\n");
		out_str("\tstatics, numbers, redcdata, "
		       "strings, literals\n");
		rescantoken();
		return;
...
/* @krc@ */
	...
	if (p == NULL) {
		err_str("Cannot determine HOME directory\n");
	}
