...
endfunc(void)
{
	...
	if (newname[0] != '*' && (conf->traceflags & TRACE_FNCODES)) {
		...
		for (size = 0; size < fp->f_opcodecount; ) {
			out_fmt("%ld: ", (unsigned long)size);
			size += dumpop(&fp->f_opcodes[size]);
		}
	}
	/* @krc@ inputisterminal() returns FALSE */
	if (/* @krc@ (inputisterminal() && conf->resource_debug & RSCDBG_STDIN_FUNC) || */
	    (/* @krc@ !inputisterminal() && */ conf->resource_debug & RSCDBG_FILE_FUNC)) {
		out_fmt("%s(", newname);
		for (index = 0; index <	 fp->f_paramcount; index++) {
			if (index)
				putchar(',');
			out_fmt("%s", paramname(index));
		}
		out_str(") ");
