...
scanerror(int skip, char *fmt, ...)
{
	...
	if ((!c_flag && !stoponerror) || stoponerror > 0) {
		if (calc_use_scanerr_jmpbuf != 0) {
			...
			err_str("calc_scanerr_jmpbuf not setup, exiting code 60\n");
	...
	if (conf->maxscancount > 0 && errorcount > conf->maxscancount) {
		err_str("Too many scan errors, compilation aborted.\n");
		if (calc_use_scanerr_jmpbuf != 0) {
			longjmp(calc_scanerr_jmpbuf, 61);
			/*NOTREACHED*/
		} else {
			err_str("calc_scanerr_jmpbuf not ready: exit 61\n");
			libcalc_call_me_last();
			exit(61);
		}
	}
...
warning(char *fmt, ...)
{
	...
	if (calc_print_scanwarn_msg != 0) {
        char msg_intro[] = "Warning: ";
        char* cp = (char*) malloc(strlen(msg_intro) + strlen(calc_warn_msg) + 3);
        strcpy(cp, msg_intro);
        strcat(cp, calc_warn_msg);
        strcat(cp, "\n");
		err_str(cp);
		free(cp);
	}
