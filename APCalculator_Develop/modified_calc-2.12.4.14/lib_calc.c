 ...
 * global permissions
 */
int allow_read = FALSE /* @krc@ TRUE */;	/* FALSE => may not open any files for reading */
int allow_write = FALSE /* @krc@ TRUE */; /* FALSE => may not open any files for writing */
...
/* @krc@ int stdin_tty = FALSE;	*/	/* TRUE if stdin is a tty */
...
/* @krc@ jmp_buf calc_matherr_jmpbuf; @krc@ */
...
/* @krc@ E_FUNC void file_init(void); @krc@ */
...
/*#if !defined(_WIN32)
	...
	if (conf->calc_debug & CALCDBG_RUNSTATE) {
		out_fmt("libcalc_call_me_first: run_state from %s to %s\n",
...
cvmalloc_error(char *message)
{
	/* firewall */
	if (message == NULL) {
		err_str("malloc_error message is NULL\n");
		return;
	}

	/* print message and return */
    char msg_intro[] = "malloc_error: ";
    char* cp = (char*)malloc(strlen(msg_intro) + strlen(message) + 3);
    strcpy(cp, msg_intro);
    strcat(cp, message);
    strcat(cp, "\n");
	err_str(cp);
	...
	for (i=0; i < fd_setup_len; ++i) {
		if (fd_setup[i] >= 0) {
			if (conf->calc_debug & CALCDBG_TTY)
				out_fmt("libcalc_call_me_last: fd %d "
...
calc_tty(int fd)
{
	...
	if (slot < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_str("calc_tty: Cannot get saved descriptor slot\n");
		return FALSE;
	}
	...
	if (fd_setup[slot] < 0 && ioctl(fd, TIOCGETP, fd_orig+slot) < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_fmt("calc_tty: Cannot TIOCGETP fd %d\n", fd);
		return FALSE;
	}
	/* setup for new state */
	...
	if (ioctl(fd, TIOCSETP, fd_cur+slot) < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_fmt("calc_tty: Cannot TIOCSETP fd %d\n", fd);
		return FALSE;
	}
	if (conf->calc_debug & CALCDBG_TTY)
		out_fmt("calc_tty: stty -ECHO +CBREAK: fd %d\n", fd);
	...
	if (fd_setup[slot] < 0 && ioctl(fd, TCGETA, fd_orig+slot) < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_fmt("calc_tty: Cannot TCGETA fd %d\n", fd);
		return FALSE;
	}
	/* setup for new state */
	fd_cur[slot] = fd_orig[slot];
	...
	if (ioctl(fd, TCSETAW, fd_cur+slot) < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_fmt("calc_tty: Cannot TCSETAW fd %d\n", fd);
		return FALSE;
	}
	if (conf->calc_debug & CALCDBG_TTY)
		out_fmt("calc_tty: stty -ECHO -ECHOE -ECHOK -ICANON +ISTRIP "
		       "VMIN=1 VTIME=0: fd %d\n", fd);
	...
	if (fd_setup[slot] < 0 && tcgetattr(fd, fd_orig+slot) < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_fmt("calc_tty: Cannot tcgetattr fd %d\n", fd);
		return FALSE;
	}
	/* setup for new state */
	fd_cur[slot] = fd_orig[slot];
	...
	if (tcsetattr(fd, TCSANOW, fd_cur+slot) < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_fmt("calc_tty: Cannot tcsetattr fd %d\n", fd);
		return FALSE;
	}
	if (conf->calc_debug & CALCDBG_TTY)
		out_fmt("calc_tty: stty -ECHO -ECHOE -ECHOK -ICANON +ISTRIP "
...
orig_tty(int fd)
{
	...
	if (slot < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_str("orig_tty: Cannot get saved descriptor slot\n");
		return FALSE;
	}
	...
	if (fd_setup[slot] < 0) {
		if (conf->calc_debug & CALCDBG_TTY)
			out_fmt("orig_tty: no state saved for fd %d\n", fd);
		return FALSE;
	}
	...
	if (conf->calc_debug & CALCDBG_TTY)
		out_fmt("orig_tty: TIOCSETP restored fd %d\n", fd);
	...
	if (conf->calc_debug & CALCDBG_TTY)
		out_fmt("orig_tty: TCSETAW restored fd %d\n", fd);
	...
	if (conf->calc_debug & CALCDBG_TTY)
		out_fmt("orig_tty: TCSANOW restored fd %d\n", fd);

#else /* nothing assigned */

	if (conf->calc_debug & CALCDBG_TTY)
		out_fmt("orig_tty: nothing restored to fd %d\n", fd);
