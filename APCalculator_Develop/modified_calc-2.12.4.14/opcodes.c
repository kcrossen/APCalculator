...
o_quit(FUNC *fp, long index)
{
	...
	/* @krc@ Never true, inputisterminal() returns FALSE
	if (inputisterminal() && !strcmp(fp->f_name, "*")) {
		...
		*//* @krc@ hist_term(); *//*
		...
		if (calc_use_scanerr_jmpbuf != 0) {
			...
			err_str("calc_scanerr_jmpbuf not setup, exiting code 50\n");
	...
	*/
	if (cp)
		...
		out_str("quit or abort executed\n");
	/* @krc@ !inputisterminal() always TRUE, inputisterminal() returns FALSE */
	if (/* @krc@ !inputisterminal() && */ !strcmp(fp->f_name, "*"))
		closeinput();
	go = FALSE;
}
...
o_show(FUNC *fp, long arg)
{
	...
	case 7: /* @krc@ showfiles(); @krc@ */ return;
	...
	fp = findfunc(arg - 19);
	if (fp == NULL) {
		out_str("Function not defined\n");
		return;
	}
	dumpnames = FALSE;
	for (size = 0; size < fp->f_opcodecount; ) {
		out_fmt("%ld: ", (long)size);
...
showsizes(void)
{
	...
	out_fmt("\tlong\t\t%4ld\n", (long)sizeof(long));
	out_fmt("\tpointer\t\t%4ld\n", (long)sizeof(void *));
	/* @krc@ out_fmt("\tFILEPOS\t\t%4ld\n", (long)sizeof(FILEPOS)); */
	/* @krc@ out_fmt("\toff_t\t\t%4ld\n", (long)sizeof(off_t)); */
	out_fmt("\tHALF\t\t%4ld\n", (long)sizeof(HALF));
	out_fmt("\tFULL\t\t%4ld\n", (long)sizeof(FULL));
	out_fmt("\tVALUE\t\t%4ld\n", (long)sizeof(VALUE));
	out_fmt("\tNUMBER\t\t%4ld\n", (long)sizeof(NUMBER));
	...
	out_fmt("\tMATRIX\t\t%4ld\n", (long)sizeof(MATRIX));
	out_fmt("\tLIST\t\t%4ld\n", (long)sizeof(LIST));
	out_fmt("\tLISTELEM\t%4ld\n", (long)sizeof(LISTELEM));
	out_fmt("\tOBJECT\t\t%4ld\n", (long)sizeof(OBJECT));
	out_fmt("\tOBJECTACTIONS\t%4ld\n", (long)sizeof(OBJECTACTIONS));
	out_fmt("\tASSOC\t\t%4ld\n", (long)sizeof(ASSOC));
	out_fmt("\tASSOCELEM\t%4ld\n", (long)sizeof(ASSOCELEM));
	out_fmt("\tBLOCK\t\t%4ld\n", (long)sizeof(BLOCK));
	out_fmt("\tNBLOCK\t\t%4ld\n", (long)sizeof(NBLOCK));
	out_fmt("\tCONFIG\t\t%4ld\n", (long)sizeof(CONFIG));
	/* @krc@ out_fmt("\tFILEIO\t\t%4ld\n", (long)sizeof(FILEIO)); */
	out_fmt("\tRAND\t\t%4ld\n", (long)sizeof(RAND));
	out_fmt("\tRANDOM\t\t%4ld\n", (long)sizeof(RANDOM));
}
...
calculate(FUNC *fp, int argcount)
{
	...
	while (go) {
		...
		if (conf->traceflags & TRACE_OPCODES) {
			dumpnames = FALSE;
			out_fmt("%8s, pc %4ld:  ", fp->f_name, pc);
...
dumpop(unsigned long *pc)
{
	...
	case OP_QUIT: case OP_ABORT:
		if ((long)(*pc) >= 0)
		    out_fmt(" \"%s\"", findstring((long)(*pc))->s_str);
		putchar('\n');
		return 2;
	case OP_INDEXADDR:
		out_fmt(" %ld %ld\n", pc[0], pc[1]);
