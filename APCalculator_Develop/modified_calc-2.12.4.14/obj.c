...
showobjfuncs(void)
{
	...
	out_str("Note: xx represents the actual object type name.\n\n");
	out_str("Name	Args	Comments\n");
...
showobjtypes(void)
{
	...
	ep = &elements;
	if (hp->h_count == 0) {
		out_str("No object types defined\n");
		return;
	}
	for (index = 0; index < hp->h_count; index++) {
		oap = objects[index];
		out_fmt("\t%s\t{", namestr(&objectnames, index));
		for (i = 0; i < oap->oa_count; i++) {
			if (i) out_str(",");
			out_fmt("%s", namestr(ep, oap->oa_elements[i]));
		}
		out_str("}\n");
	}
